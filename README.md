...
Twitch 待命畫面 / 正片切換監控。版本 **v0.15.0**。版本號避開 4。
...
可調：`FRAME_INTERVAL_SEC`（預設 3）、`AD_SKIP_SEC`（20）、`CONFIRM_FRAMES`（連續 4 張不像待命才通知）。**開頭偵測時限**在設定裡填幾小時幾分，預設 1 小時 0 分（開台超過就不再抽幀判定正片；兩個都 0＝不略過）。每台的相似度與略過色存在 `watchlist.json`。
